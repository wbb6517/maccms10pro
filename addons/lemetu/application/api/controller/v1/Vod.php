...
//        $info = $this->label_vod_detail($labelParam);
        ...
        if ($res['code'] == 1){
            ...
            $relVods = [];
            $info = $res['info'];
            $info['vod_points_play'] = intval(config('app.app.trysee_points'));
            ...
            if($GLOBALS['user']['group_id']!=3){
                if(!empty($userData['user_view_count'])){
                    // 播放记录
                    $used_times = $this->getUserVideoTimes($GLOBALS['user']['user_id']);
                    if($used_times<=0){
                        $data['user_video'] = 0;
                        return $this->success($data);
                    }
    ...
    public function postScore()
    {
        ...
        $param = input();
        // Log::info('postScore_test'.json_encode($param));
        file_put_contents('postScore_test.txt', '['.date('c').']postScore_test'.json_encode($param) , FILE_APPEND);
        $id = $param['id'];
//        $mid = $param['mid'];
        $mid = 1;
        $score = $param['score'];
...
//        $mids = [1=>'vod',2=>'art',3=>'topic',8=>'actor',9=>'role'];
        ...
        $info = $res['info'];
        $add_score = 0;
        file_put_contents('postScore_test.txt', '['.date('c').']$info'.json_encode($info), FILE_APPEND);
        if ($info) {
            if($score){
//                $cookie = $pre.'-score-'.$id;
...
//                }
                ...
                file_put_contents('postScore_test.txt', '['.date('c').']Ulog_$res'.json_encode($res), FILE_APPEND);
                if ($res['code'] == 1){
                    // 发放评分奖励 
                    //$config = config('app.task')['mark'];
                    $config = config('app')['task']['mark'];
                    file_put_contents('postScore_test.txt', '['.date('c').']task_mark'.json_encode($config), FILE_APPEND);
                    if ($config['status'] == 1){
                        ...
                        if ($reword['points'] > 0){
                            ...
                            if ($config['reward_num']){
                               ...
                               // $exsPlogsCount = model('Plog')->countData($where);
                                $exsPlogsCount = model('Plog')->where($where)->count();
                               
                                
                            }
                            
                            file_put_contents('postScore_test.txt', '['.date('c').']reward_num'.json_encode($config) . ' '.$exsPlogsCount , FILE_APPEND);
...
//  首页视频推荐
        ...
        $data = (object)[
            'zhui'=>[
                [
                    ...
                    'vod_type_id'=>1,
                    'vod_type_name'=>$data['tuijian1'],
                    'vod_type_img'=>$data['tuijiansimg1'],
                    'vod_list'=>model('Vod')->listData(['vod_level'=>$data['tuijians1']],$types[$phbType].' desc',1,12)['list'],
                ]
            ],
            'list'=>[
                [
                    'vod_type_id'=>1,
                    'vod_type_name'=>$data['tuijian2'],
                    'vod_type_img'=>$data['tuijiansimg2'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>qq,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>$data['tuijians2'],'vod_year'=> 2020],'vod_time desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['type_id'=>$data['tuijians2'],'vod_year'=>$data['tuijanss2']],'vod_time desc',1,12)['list'],
                ],
                [
                    'vod_type_id'=>2,
                    'vod_type_name'=>$data['tuijian3'],
                    'vod_type_img'=>$data['tuijiansimg3'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>qiyi,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>$data['tuijians3'],'vod_year'=>2021],'vod_time desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['type_id'=>$data['tuijians3'],'vod_year'=>$data['tuijanss3']],'vod_time desc',1,12)['list'],
                ],
                [
                    'vod_type_id'=>3,
                    'vod_type_name'=>$data['tuijian4'],
                    'vod_type_img'=>$data['tuijiansimg4'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>youku,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>$data['tuijians4'],'vod_year'=>2020],'vod_time desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['type_id'=>$data['tuijians4'],'vod_year'=>$data['tuijanss4']],'vod_time desc',1,12)['list'],

                ],
                [
                    'vod_type_id'=>4,
                    'vod_type_name'=>$data['tuijian5'],
                    'vod_type_img'=>$data['tuijiansimg5'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>mgtv,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>$data['tuijians5'],'vod_year'=>2020],'vod_time desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['type_id'=>$data['tuijians5'],'vod_year'=>$data['tuijanss5']],'vod_time desc',1,12)['list'],
    ...
    public function postVideoViewRecode()
    {
        ...
        $user_video = $this->getUserVideoTimes($user_id);
        $data = [
     ...
     *
     */
    private function getUserVideoTimes($userId){
        if($userId == 0){
            return 0;
        }
        $data=[];
