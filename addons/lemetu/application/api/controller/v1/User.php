...
class User extends Base {
    ...
    public function getDetail(){
        ...
        $data['leave_times'] = $this->getUserVideoTimes($user_id);
        return $this->success($data);
    }
    ...
    public function postIndex()
    {
        ...
        $param = input();
        if (Request()->isPost()) {
            $res = model('User2')->fieldData(['user_id' => $GLOBALS['user']['user_id']] ,$param,'');
            if ($res['code'] == 1) {
    ...
    public function postBuy()
    {
        ...
        if (Request()->isPost()) {
            ...
            if ($flag == 'card') {
                ...
                $data['order_code'] = 'PAY' . $this->ec_get_uniqid_code();
    ...
    public function postBuypopedom()
    {
        ...
        if ($param['pay_type'] == 1 && config('app.app')['trysee_points'] > $GLOBALS['user']['user_points']) {
            return $this->error('对不起,查看此页面数据需要[' . config('app.app')['trysee_points'] . ']积分，您还剩下[' . $GLOBALS['user']['user_points'] . ']积分，请先充值！');
        } else {
            switch ($param['pay_type']){
                case 1:
                    $where = [];
                    $where['user_id'] = $GLOBALS['user']['user_id'];
                    $res = model('User2')->where($where)->setDec('user_points',$data['ulog_points']);
                    if ($res === false) {
...
//            model('User')->reward($data['ulog_points']);
                    $end_time = config('app.app.trysee_time');
                ...
                case 2:
                    $data2 = [];
                    $data2['user_id'] = $GLOBALS['user']['user_id'];
                    $data2['order_code'] = 'PAY' . $this->ec_get_uniqid_code();
                ...
                case 3:
                    ...
                    $data3['order_code'] = 'PAY' . $this->ec_get_uniqid_code();
...
//        $this->assign('param',$param);
    ...
    public function postChangeAgents()
    {
		...
		$score = config('app')['app']['agents_score'];
        $info = model('User2')->changeAgents($where, $score);
    ...
    public function getCheckVodTrySee()
    {
        ...
        $info['user_video'] = $this->getUserVideoTimes($user['user_id']);
    ...
    {
        $this->checkLogin();
        $gold_min = intval(config('app')['user']['gold_min']);
        $gold_ratio = intval(config('app')['user']['gold_ratio']);
        $user = $GLOBALS['user'];
    ...
    {
        $this->checkLogin();
        $score = config('app.task')['share']['reward']['points'];
        $reword_num = config('app.task')['share']['reward_num'];
        $user = $GLOBALS['user'];
        ...
        model('User2')->where('user_id', $user['user_id'])->setInc('user_points', $score);
        $data=[
            'user_id'=>$user['user_id'],
            'plog_type'=>'15',
            'plog_points' => $score,
    ...
    {
        //$this->checkLogin();
        $macConfig = config('app');
        $data =[
            'share_url' => $macConfig['app']['share_url']. ($GLOBALS['user']['user_id'] ? '?invite_code='. \app\common\model\User2::getInviteCode($GLOBALS['user']['user_id']) : ''),
            'share_logo' => $macConfig['app']['share_logo'] ? $this->ec_path_to_imageurl($macConfig['app']['share_logo']) : null,
        ];
        return $this->success($data);
    }

    public function ec_path_to_imageurl($path){
        $preg = "/^http(s)?:\\/\\/.+/";
    ...
    public function getSubUsers()
    {
        ...
        $res = model('User2')->listData($where,'user_id desc',$page,$limit,$field);
        $data = [
            'limit' => $res['limit'],
            'page' => $res['page'],
            'total' => $res['total'],
    ...
    private function ec_get_uniqid_code($code_prefix='')
    {
