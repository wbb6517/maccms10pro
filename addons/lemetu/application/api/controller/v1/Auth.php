    ...
    public function postWxlogin()
    {
        ...
        if (is_null($row)){
            ...
            if( $GLOBALS['config']['user']['reg_num'] > 0){
                ...
                $cc = model('user2')->where($where2)->count();
            ...
            $nid = model('user2')->insertGetId($fields);
            if (empty($nid))
                $this->error('注册失败');
...
//        cookie('user_portrait', mac_get_user_portrait($row['user_id']),['expire'=>2592000] );
    ...
    public function postRegister(){
        if (Request()->isPost()) {
            ...
            $res = model('User2')->register($param);
            $code = $res['code'];
            if($code == 1){
                $this->success();
            }else{
     ...
     * 找回密码
     */
    public function getFindPass2(){
        if (Request()->isGet()) {
            $param = input();
            // $param['uid']  = cookie('invite_by_user_id');
            Log::mylog('找回密码参数：', $param, 'reg');
            $res = model('User2')->info($param);
            $code = $res['code'];
            if($code == 1){
                $this->success();
            }else{
 ...
 /**
    ...
    /*public function getFindPass(){
        if (Request()->isGet()) {
            ...
            // $param['uid']  = cookie('invite_by_user_id');
            Log::mylog('找回密码参数：', $param, 'reg');
            $res = model('User2')->findpass_reset($param);
            $code = $res['code'];
            if($code == 1){
                $this->success();
            }else{
    ...
    {
        $param = input();
        $res = model('User2')->findpass_msg($param);
        if($res['code'] == 1){
            $this->success();
        }else{
            $this->error($res['msg']);
        ...
        $times++;
        $prefix = config('app')['app']['reg_autoname_prefix'];
        // $name = uniqid($prefix);
