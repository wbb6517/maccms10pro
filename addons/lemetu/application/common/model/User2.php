...
//        $this->reward($point);
    ...
    public function reward($where, $fee_points=0)
    {
        ...
        $user = model('User2')->where($where)->find();
        if (!$user){
        ...
        if($fee_points>0 && $GLOBALS['config']['user']['reward_status'] == '1'){
            ...
            if(!empty($GLOBALS['config']['user']['reward_ratio']) && !empty($user['user_pid'])){            // 如果奖励开启 并且用户存在上级
                ...
                if($points>0){
                    ...
                    $r1 = model('User2')->where($where)->find($where);

                    if ($r1){
                        // 查看是否是代理商 返金币
                        if (self::isAgents($r1)){
                            $r = model('User2')->where($where)->setInc('user_gold',$points);
                            if($r){
                                $data = [];
                            ...
                            $r = model('User2')->where($where)->setInc('user_points',$points);
                            if($r){
                                $data = [];
                                $data['user_id'] =$user['user_pid'];
                                $data['plog_type'] = 4;
            ...
            if(!empty($GLOBALS['config']['user']['reward_ratio_2']) && !empty($user['user_pid_2'])){
                ...
                if($points>0){
                    ...
                    $r2 = model('User2')->where($where)->find($where);
            ...
            if(!empty($GLOBALS['config']['user']['reward_ratio_3']) && !empty($user['user_pid_3'])){
                ...
                if($points>0){
                    ...
                    $r3 = model('User2')->where($where)->find($where);
                    if (self::isAgents($r3)){
                        $r = model('User2')->where($where)->setInc('user_gold',$points);
                        if($r){
                            $data = [];
